...
#define IntervaloRPM  40000  // Define o valor do intervalo de tempo para calculo do rpm (Dado em microsegundos)
...
Neotimer loopDados = Neotimer(4); // Envio dos dados para a tela
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
const int numPontos = 512; // Define o tamanho do array (quantidade de pontos da função sin)
...
void setup() {
  ...
  //Serial.println("Tempo (µs),Ciclo_CPU (µs),Referência (RPM),Vel_Real (RPM)");
...
void loop(){
  ...
  calculaRPM();
  if(ligaMotor){
    refMotor0 = sensorValueA8 * (2400 / 1023.0); //Referência do motor de 0 a 2400 rpm
    velmotor0 = sensorValueA8 * (255 / 1023.0); //Calcula velocidade do motor de 0 a 100% com base no pwm de 0 a 255 (gerado pelo entrada analógica A8)
  }
  else{
      refMotor0 = 0;
      //rpm = 0;
...
void ImprimeStatus(){
  if(loopDados.done()){
    ...
    Serial.print(map(refMotor0, 0, 2400, 2706, 3638));
    //Serial.print(map(velmotor1, 140, 240, 1710, 2120));//(10*velmotor1);
    Serial.print(",");
